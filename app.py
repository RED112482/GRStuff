...
def _latest_matching_scan(
    ...
    for scan in reversed(sequence):
        if (
            max_sequence_index is not None
            and int(scan["sequence_index"]) > max_sequence_index
        ):
            continue
        if _angle_matches(scan["elevation"], elevation):
            return scan
...
def _wall_scan_for_elevation(
    ...
    if anchor_source == "live":
        ...
        # The live volume has not reached this elevation yet. Keep showing the
        # latest available scan from the most recent completed volume.
        for archive_index, entry in enumerate(archive_history[:2]):
            if current_live_time and entry["volume_time"]:
                if abs(
                    (entry["volume_time"] - current_live_time).total_seconds()
                ) < 30:
                    continue
